...
router.get("/", function(req, res) {
    const exec = async() => {
        ...
        const res1 = await promise("select u.username, TO_CHAR(c.wakeup_date, 'MM/DD HH24:MI') wakeup_date, c.comment, c.call_id from call_orders c join users u using (user_id) where c.deleted = false and c.wakeup_date > current_timestamp + interval '+9:00' order by wakeup_date asc;", )
        let data = {
            items: res1,
        }
